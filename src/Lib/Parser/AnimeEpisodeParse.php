...
class AnimeEpisodeParse extends TemplateParse
{
    ...
    public function parse(): Array
    {
        ...
        $this->addRule(
            'episode',
            '~<table border="0" cellspacing="0" cellpadding="0" width="100%" class="mt8 episode_list js-watch-episode-list ascend">~',
            function () {
                $running = true;
                $i = 1;
                ...
                while ($running) {
                    ...
                    if (preg_match('~<tr class="episode-list-data">~', $line)) {
                        ...
                        preg_match(
                            '~<td class="episode-number nowrap">(.*)</td>~',
                        ...
                        preg_match(
                            '~<td class="episode-video nowrap"><a href="(.*)"><img src="(.*)" width="20" height="19" alt="(.*)"></a></td>~',
                            $this->file[$this->lineNo + $i],
                            $this->matches
                        );
                        ...
                        preg_match(
                            '~<td class="episode-title">(<span class="fl-r di-ib pr4 icon-episode-type-bg">Filler</span>|<span class="fl-r di-ib pr4 icon-episode-type-bg">Recap</span>|)<a href="(.*)" class="fl-l fw-b ">(.*)</a>~',
                            $this->file[$this->lineNo + $i],
                            $this->matches
                        );
                        $epMeta['title'] = $this->matches[3];

                        $epMeta['filler'] = preg_match(
                            '~<span class="fl-r di-ib pr4 icon-episode-type-bg">Filler</span>~',
                            $this->file[$this->lineNo + $i]
                        ) ? true : false;
                        $epMeta['recap'] = preg_match(
                            '~<span class="fl-r di-ib pr4 icon-episode-type-bg">Recap</span>~',
                            $this->file[$this->lineNo + $i]
                        ) ? true : false;
                        $i++;
                        ...
                        preg_match(
                            '~<td class="episode-aired nowrap">(.*)</td>~',
                            $this->file[$this->lineNo + $i],
                            $this->matches
                        );
                        ...
                        preg_match(
                            '~<a href="(.*)"><i class="fa fa-comments mr4"></i>Forum</a></td>~',
                            $this->file[$this->lineNo + $i],
                            $this->matches
                        );
